...
<div class="container py-5" style="font-family: 'Inter', sans-serif;">
  <div class="wrap" style="display: grid; grid-template-columns: 320px 1fr; gap: 20px; max-width: 980px; margin: 0 auto;">
    ...
    {{-- LEFT: PROFILE CARD --}}
    <aside class="card profile text-center" style="display: flex; flex-direction: column; align-items: center; position: relative; background: linear-gradient(180deg, #fff7f2 0%, #ffe8d9 100%); overflow: hidden; border-radius: 14px; padding: 20px;">

      {{-- Avatar Huruf --}}
      <div class="avatar-placeholder" style="width: 110px; height: 110px; border-radius: 20px; background: linear-gradient(135deg, #ff7a2d, #e06629); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 36px; box-shadow: 0 6px 18px rgba(255,122,45,0.18); margin-bottom: 15px;">
      ...
      <div class="mt-3 small text-muted">
        ...
        <p><i class="bi bi-geo-alt"></i> {{ $user->provinsi ?? '-' }}, {{ $user->kota ?? '-' }}</p>
        <p><i class="bi bi-house-door"></i> {{ $user->alamat ?? 'Alamat belum diisi' }}</p>
        <p><i class="bi bi-telephone"></i> {{ $user->kontak ?? '-' }}</p>
      </div>
    ...
    <main class="card" style="background: #fff; border-radius: 14px; box-shadow: 0 6px 20px rgba(34,34,34,0.08); padding: 22px;">
      ...
      <p><strong>Kota/Kabupaten:</strong> {{ $user->kota ?? '-' }}</p>
      <p><strong>Alamat Lengkap:</strong> {{ $user->alamat ?? '-' }}</p>
