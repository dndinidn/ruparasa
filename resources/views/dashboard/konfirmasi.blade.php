    ...
    @if($pesanan && $pesanan->items->count() > 0)
        @php
            $stokHabis = $pesanan->items->contains(function($i){
                return $i->produk->stok == 0;
            });
            $subtotal = $pesanan->items->sum(fn($i) => $i->harga * $i->jumlah);
            $ongkir = $pesanan->ongkir ?? 15000;
        ...
        <div class="card p-4">
            ...
            @if($stokHabis)
                <p class="text-danger mt-2">Ada produk yang stoknya habis. Silakan hapus atau ubah jumlah sebelum checkout.</p>
            @endif
        </div>
    @else
        <p class="text-center text-muted py-4">Keranjang kosong. Tidak bisa checkout.</p>
    @endif
</div>
...
document.getElementById('btnBayar')?.addEventListener('click', function() {
    ...
    // Cek stok habis sebelum checkout
    @if($stokHabis)
        Swal.fire({
            icon: 'error',
            title: 'Gagal!',
            text: 'Ada produk dengan stok 0, tidak bisa checkout.',
